    ...
    <div class="modal fade" id="detailsModal" tabindex="-1" role="dialog" aria-labelledby="detailsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
            ...
            <div class="modal-body ps-5 pe-5">
                ...
                <p><strong>Email:</strong> <span id="em
                ail-details"></span></p>
            </div>
...
<div class="content-container">
    <div class="container mt-3">
        <div class="my-2" style="display: flex; justify-content: center; align-items: center;">
            <h1>View Barangay Info</h1>
        </div>
        <div style="text-align: center;">
            <input type="number" id="searchBar" placeholder="Search by Barangay ID..." class="form-control mb-3 mx-auto" onkeyup="searchTable()" style="border-radius: 5px; padding: 10px; width: 50%;">
            <p id="noResults" style="display: none; color: red; font-size: 18px;">No search results.</p>
        </div>
        
        <div class="scrollable-table-container"> <!-- Scrollable table container -->
            <table class="table table-striped table-bordered text-center" id="barangayTable">
...
while ($rowBarangays = mysqli_fetch_assoc($resultBarangays)) {
    ...
    $district = getDistrict($barangayId);
    $zone = getZone($district, $barangayId);

    $content .= <<<EOD
                    <tr>
                        <td>{$rowBarangays['id']}</td>
                        <td>{$rowBarangays['barangay_name']}</td>
                        <td>{$district}</td>
                        <td>Zone {$zone}</td>
                        <td>
                            <form method="post" class="d-flex justify-content-center">
                                <input type="hidden" name="selectedBarangayId" value="{$rowBarangays['id']}">
                                <button type="submit" name="viewResidents" class="btn btn-success mr-1 me-4">View</button>
                                <button type="submit" id="editPasswordBtn" name="editPassword" class="btn btn-warning border" style="height: 42px;">Edit Password</button>
                            </form>
                        </td>
                    </tr>
EOD;
